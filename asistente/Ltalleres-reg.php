...
while ($Qf_evento = mysql_fetch_array($fechaRecords))
	{
		...
		$QAins='SELECT E.id_propuesta, P.duracion, P.id_ponente, EO.id_lugar, EO.id_fecha, E.id_propuesta, AI.id_evento, PO.nombrep, PO.apellidos, P.nombre, O.descr, EO.hora, L.nombre_lug FROM ponente AS PO, lugar AS L, orientacion AS O, inscribe AS AI, evento AS E, propuesta AS P, evento_ocupa AS EO  WHERE AI.id_evento=E.id AND E.id_propuesta=P.id AND AI.id_evento=EO.id_evento AND P.id_orientacion=O.id AND EO.id_lugar=L.id AND P.id_ponente=PO.id AND EO.id_fecha="'.$Qf_evento[id].'" AND AI.id_asistente="'.$idasistente.'" GROUP BY AI.id_evento ORDER BY EO.hora';
		...
		while ($Qf_event= mysql_fetch_array($AIeventoRecords))
		{
			...
			print '</td><td bgcolor='.$bgcolor.'><small><a class="rojo" href="Btaller.php?vact='.$Qf_event[id_evento].' '.$REQUEST_URI.'" onMouseOver="window.status=\'Dar de baja\';return true" onFocus="window.status=\'Dar de baja\';return true" onMouseOut="window.status=\'\';return true">Dar de baja</a>';
			print '</td></tr>';
